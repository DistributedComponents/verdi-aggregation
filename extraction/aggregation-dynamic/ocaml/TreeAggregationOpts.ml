...
let read_mic_timeout_default = 2.0
let device_default = 5
let channels_default = 2
...
let read_mic_timeout = ref read_mic_timeout_default
let device = ref device_default
let channels = ref channels_default
...
let parse inp =
  let opts =
    ...
    ; ("-read-mic-timeout", Arg.Set_float read_mic_timeout, "{seconds} timeout for reading microphone data")    
    ; ("-device", Arg.Set_int device, "{id} identifier for microphone device")
    ; ("-channels", Arg.Set_int channels, "{count} number of channels for device")
    ] in
