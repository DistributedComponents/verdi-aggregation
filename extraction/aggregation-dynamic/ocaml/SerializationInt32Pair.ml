...
let deserializeInput (s : string) (c : string) : coq_Input option =
  ...
  | _ -> 
    try Scanf.sscanf s "Local %ld %ld" (fun x y -> Some (Local (Obj.magic (Obj.magic x, Obj.magic y))))
    with _ -> None

let serializeOutput : coq_Output -> string * string = function
  | AggregateResponse (c, x) -> (string_of_char_list c, Printf.sprintf "AggregateResponse %ld %ld" (fst (Obj.magic x)) (snd (Obj.magic x)))
...
let debugSerializeInput : coq_Input -> string = function
  ...
  | Local x -> Printf.sprintf "Local %ld %ld" (fst (Obj.magic x)) (snd (Obj.magic x))
  | LevelRequest x -> Printf.sprintf "LevelRequest %s" (string_of_char_list x)

let debugSerializeMsg : coq_Msg -> string = function
  | New -> "New"
  | Aggregate x -> Printf.sprintf "Aggregate %ld %ld" (fst (Obj.magic x)) (snd (Obj.magic x))
  | Fail -> "Fail"
