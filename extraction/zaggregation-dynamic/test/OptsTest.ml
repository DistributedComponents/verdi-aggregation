...
let test_parse_correct_line test_ctxt =
  Opts.parse (arr_of_string "./vard.native -me 0 -port 8000 -node 0,localhost:9000 -node 1,localhost:9001 -node 2,localhost:9002");
...
let test_validate_missing_me text_ctxt =
  Opts.parse (arr_of_string "./vard.native -port 8000 -node 0,localhost:9000 -node 1,localhost:9001 -node 2,localhost:9002");
  assert_raises (Arg.Bad "Please specify the node name -me") Opts.validate

let test_validate_empty_cluster text_ctxt =
  Opts.parse (arr_of_string "./vard.native -me 0 -port 8000");
  assert_raises (Arg.Bad "Please specify at least one -node") Opts.validate

let test_validate_me_not_cluster_member text_ctxt =
  Opts.parse (arr_of_string "./vard.native -me 0 -port 8000 -node 1,localhost:9001 -node 2,localhost:9002");
  assert_raises (Arg.Bad "0 is not a member of this cluster") Opts.validate

let test_validate_duplicate_node_entry text_ctxt =
  Opts.parse (arr_of_string "./vard.native -me 0 -port 8000 -node 0,localhost:9000 -node 0,localhost:9001");
